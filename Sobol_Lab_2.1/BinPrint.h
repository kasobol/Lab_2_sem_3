...
class BinTree
{
    struct Node
    {
        ...
        Node(const TKey& key, const T& value) :key(key), value(value), left(nullptr), right(nullptr) {}
...
public:
    ...
    void insert(const TKey& key, const T& value, Args...more)
    {
        if (!root)
        {
